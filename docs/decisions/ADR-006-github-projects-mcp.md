...
2. **Official GitHub MCP server**: GitHub released `@github/github-mcp-server` as their official
   MCP server, providing a stable, maintained tool surface for Projects v2, Issues, and PRs.
...
Use `@github/github-mcp-server` (official GitHub MCP server) run as a local stdio subprocess by
the NestJS backend, acting as an MCP client via `@modelcontextprotocol/sdk`'s `StdioClientTransport`.
...
**Architecture**:
- `GithubMcpModule` (global NestJS singleton) spawns the `@github/github-mcp-server` subprocess
  on `OnModuleInit`; terminates it on `OnApplicationShutdown`.
...
**Risk**:
- `@github/github-mcp-server` tool names could change between releases; pinning to a specific
  version in `package.json` and `mcp.json` mitigates silent breakage.
